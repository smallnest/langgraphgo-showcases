...
const (
	...
	SystemPromptReportStructure = `你是一位专业的舆情分析师和报告架构师。给定一个查询，你需要规划一个全面、深入的舆情分析报告结构。
...
只返回JSON对象，不要有解释或额外文本。`
	...
	SystemPromptFirstSearch = `你是一位专业的舆情分析师。你将获得报告中的一个段落，其标题和预期内容将按照以下JSON模式定义提供：
...
只返回JSON对象，不要有解释或额外文本。`
	...
	SystemPromptFirstSummary = `你是一位专业的舆情分析师和深度内容创作专家。你将获得丰富的真实社交媒体数据，需要将其转化为深度、全面的舆情分析段落：
...
只返回JSON对象，不要有解释或额外文本。`
	...
	SystemPromptReflection = `你是一位资深的舆情分析师。你负责深化舆情报告的内容，让其更贴近真实的民意和社会情感。你将获得段落标题、计划内容摘要，以及你已经创建的段落最新状态：
...
只返回JSON对象，不要有解释或额外文本。`
	...
	SystemPromptReflectionSummary = `你是一位资深的舆情分析师和内容深化专家。
...
只返回JSON对象，不要有解释或额外文本。`
	...
	SystemPromptReportFormatting = `你是一位资深的舆情分析专家和报告编撰大师。你专精于将复杂的民意数据转化为深度洞察的专业舆情报告。
你将获得以下JSON格式的数据：
