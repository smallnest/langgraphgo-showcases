class OpenNotebook {
    ...
    renderNotebookCards() {
        ...
        this.notebooks.forEach(nb => {
            ...
            // 直接使用后端返回的统计信息，无需额外 API 调用
            const sourcesCount = nb.sources_count ?? 0;
            const notesCount = nb.notes_count ?? 0;
            card.querySelector('.stat-sources').textContent = `${sourcesCount} 来源`;
            card.querySelector('.stat-notes').textContent = `${notesCount} 笔记`;
    ...
    // 更新笔记本卡片上的统计信息（使用已有的 DOM 数据，无需额外 API 调用）
    updateNotebookCardStats() {
        if (!this.currentNotebook) return;

        const notebookCard = document.querySelector(`.notebook-card[data-id="${this.currentNotebook.id}"]`);
        if (notebookCard) {
            // 直接从 DOM 中统计当前显示的来源和笔记数量
            const sourceCount = document.querySelectorAll('.source-card').length;
            const noteCount = document.querySelectorAll('.note-item').length;

            notebookCard.querySelector('.stat-sources').textContent = `${sourceCount} 来源`;
            notebookCard.querySelector('.stat-notes').textContent = `${noteCount} 笔记`;
        }
    }
}
