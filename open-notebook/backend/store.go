...
// ListNotebooks retrieves all notebooks with sources and notes counts
func (s *Store) ListNotebooks(ctx context.Context) ([]Notebook, error) {
	query := `
		...
		FROM notebooks n
		LEFT JOIN (
			SELECT notebook_id, COUNT(*) as sources_count
			FROM sources
			GROUP BY notebook_id
		) s ON n.id = s.notebook_id
		LEFT JOIN (
			SELECT notebook_id, COUNT(*) as notes_count
			FROM notes
			GROUP BY notebook_id
		) notes ON n.id = notes.notebook_id
	...
	for rows.Next() {
		...
		var metadataJSON string
		var createdAt, updatedAt int64
		var sourcesCount, notesCount int

		if err := rows.Scan(&nb.ID, &nb.Name, &nb.Description, &createdAt, &updatedAt, &metadataJSON,
			&sourcesCount, &notesCount); err != nil {
			return nil, err
		}

		nb.CreatedAt = time.Unix(createdAt, 0)
		nb.UpdatedAt = time.Unix(updatedAt, 0)
		nb.SourcesCount = sourcesCount
		nb.NotesCount = notesCount
